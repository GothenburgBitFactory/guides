...
\begin{frame}[fragile]\frametitle{IP Address}\label{S1}
    \vfill
    \begin{lstlisting}
ifconfig -a
ip addr list\end{lstlisting}

    Show the ip addresses (and network interfaces) on the system.
    \vfill
\end{frame}

\begin{frame}[fragile]\frametitle{Ports}\label{S2}
    \vfill
    \begin{lstlisting}
lsof -i TCP:53589 -s TCP:LISTEN # as taskserver user or with sudo
netstat -tln | grep 53589\end{lstlisting}

    Shows which process listen on the port, shows the user and - more
    important - the ip address / interface the server runs on.
    \vfill
\end{frame}

\begin{frame}[fragile]\frametitle{Certificate}\label{S3}
    \vfill
    \begin{lstlisting}
openssl x509 -noout -in server.cert.pem -subject
certtool -i --infile=server.cert.pem | grep Subject:\end{lstlisting}

    Shows the hostname from (C1) after CN=
    \vfill
\end{frame}

\begin{frame}[fragile]\frametitle{taskd diagnostics}\label{S4}
    \vfill
    \begin{lstlisting}
taskd diagnostics --data /path/to/taskddata\end{lstlisting}

    Shows server daignostic data.
    \vfill
\end{frame}

\subsubsection{Client}

\begin{frame}[fragile]\frametitle{General Reachability}\label{C1}
    \vfill
    \begin{lstlisting}
ping hostname
nmap -sP hostname\end{lstlisting}

    Show if the hostname is reachable (if ping is not blocked).
    \vfill
\end{frame}

\begin{frame}[fragile]\frametitle{Port Reachability}\label{C2}
    \vfill
    \begin{lstlisting}
telnet hostname 53589
nmap -p 53589 hostname
openssl s_client -host hostname -port 53589\end{lstlisting}

    Shows if the port is reachable by client (or locally).
    \vfill
\end{frame}

\begin{frame}[fragile]\frametitle{Hostname resolution}\label{C3}
    \vfill
    \begin{lstlisting}
host hostname
nslookup hostname # checks only nameservers\end{lstlisting}

    Shows the ip address of hostname. Should match one of the IP addresses
    found in (S1), and the server should be bound to that address (S2).
    \vfill
\end{frame}

\begin{frame}[fragile]\frametitle{task diagnostics}\label{C4}
    \vfill
    \begin{lstlisting}
task diagnostics\end{lstlisting}
